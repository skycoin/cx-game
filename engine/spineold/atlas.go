...
func NewAtlas(r io.Reader, loader TextureLoader) (*Atlas, error) {
	...
	// pageFields :=  make(map[string] int)
	...
	for scanner.Scan() {
		...
		if page == nil {
			...
			if err != nil {
				return nil, errors.New("spine: failed to parse format 1: " + err.Error())
			}
			w, _ := strconv.Atoi(size[0])
			h, _ := strconv.Atoi(size[1])

			format, err := formatFromName("RGBA8888")
			if err != nil {
				return nil, errors.New("spine: failed to parse format 2: " + err.Error())
			}

			tuple, terr = readTuple("filter", scanner)
			if terr != nil {
				return nil, errors.New("spine: failed to read page: " + terr.Error())
			}
			...
			if err := atlas.loader.Load(page, page.Name); err != nil {
...
type TextureLoader interface {
	...
	Load(page *AtlasPage, path string) error
	Unload(page *AtlasPage) error
}

// func Load(page *AtlasPage, path string) {
// 	// var texture = engi.Files.Image(path)
// 	// page.RendererObject = texture
// }

// func Unload(page *AtlasPage) {
