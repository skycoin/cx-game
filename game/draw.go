...
func Draw() {
	...
	//	actualScreenSizeWidth, actualScreenSizeHeight := glfw.GetCurrentContext().GetFramebufferSize() //*here is your old display code
	physicalViewport := render.GetCurrentViewport()
	fmt.Println("physicalViewport: ", physicalViewport)
	virtualViewport :=
		render.Viewport{
			0, 0,
			constants.VIRTUAL_VIEWPORT_WIDTH,  //int32(actualScreenSizeWidth),  //  *here is your old display code
			constants.VIRTUAL_VIEWPORT_HEIGHT, //int32(actualScreenSizeHeight), // *here is your old display code
		}
	virtualViewport.Use()
